...
async function findHeaderInfo(sheets, spreadsheetId, sheetName, requiredHeaders) {
    console.log(`[DIAGNÓSTICO] Buscando cabeçalhos [${requiredHeaders.join(', ')}] na planilha "${sheetName}"...`);
    ...
    try {
        ...
        if (rows) {
            for (let i = 0; i < rows.length; i++) {
                ...
                if (hasAllHeaders) {
                    console.log(`[DIAGNÓSTICO] Cabeçalhos encontrados na linha ${i + 1}. Headers: [${headers.join(', ')}]`);
                    return { rowIndex: i + 1, headers };
                }
            }
        }
        console.warn(`[DIAGNÓSTICO] AVISO: Nenhuma linha com os cabeçalhos [${requiredHeaders.join(', ')}] foi encontrada em "${sheetName}".`);
        return null;
    } catch (e) {
        console.error(`[DIAGNÓSTICO] ERRO CRÍTICO ao ler a planilha "${sheetName}" para encontrar cabeçalhos: ${e.message}`);
        return null;
    }
}


const createSaidaFabricaRouter = (
    ...
    router.post('/', async (req, res, next) => {
        ...
        try {
            ...
            console.log('[ETAPA 1] SUCESSO: Dados de saída inseridos na planilha de registro.');

            // ETAPA 2: Processar a baixa de estoque para cada item.
            console.log('\n[ETAPA 2] Iniciando processo de baixa de estoque...');
            const tokenResponse = await axios.get(APPS_SCRIPT_TOKEN_URL);
            const accessToken = tokenResponse.data.access_token;
            if (!accessToken) {
                throw new Error('Token de acesso do Bling não foi obtido.');
            }
            console.log('[ETAPA 2] Token de acesso do Bling obtido com sucesso.');
            ...
            if (!estoqueHeaderInfo) {
                console.error(`[ETAPA 2] ERRO CRÍTICO: Cabeçalho não encontrado na planilha de estoque. A baixa de estoque não será realizada.`);
                return res.status(200).send({
                    status: 'success_with_warning',
                    message: 'Saída registrada com sucesso, mas a baixa de estoque falhou (cabeçalho da planilha de estoque não encontrado).',
            ...
            console.log(`[ETAPA 2] Planilha de estoque lida. Total de ${estoqueRows.length} produtos encontrados.`);
            ...
            for (const item of data) {
                ...
                console.log(`\n--- Processando item: Cód: ${codigoService} | Qtd: ${quantidadeSaida} ---`);

                if (!codigoService || isNaN(quantidadeSaida) || quantidadeSaida <= 0) {
                    console.error(`[ERRO] Item com dados inválidos (código: "${codigoService}", quantidade: "${item.quantidade}"). Pulando...`);
                ...
                try {
                    // 2.1 - Atualizar Bling
                    console.log(`[Bling - ${codigoService}] Buscando ID do produto...`);
                    ...
                    console.log(`[Bling - ${codigoService}] Produto encontrado. ID: ${produtoId}. Lançando estoque...`);
                    ...
                    const blingPayload = {
                        ...
                        observacoes: `Saída via App - Requisição: ${item.requisicao}`
                    };

                    await axios.post(`${BLING_API_BASE_URL}/estoques`, blingPayload, { headers: { 'Authorization': `Bearer ${accessToken}` }});
                    console.log(`[Bling - ${codigoService}] SUCESSO: Estoque atualizado no Bling.`);

                    // 2.2 - Atualizar Planilha de Estoque
                    console.log(`[Planilha - ${codigoService}] Procurando produto para atualizar estoque...`);
                    ...
                    if (rowIndexToUpdate !== -1) {
                        ...
                        console.log(`[Planilha - ${codigoService}] Produto encontrado na linha ${rowIndexToUpdate}. Estoque atual: ${estoqueAtual}. Novo estoque: ${estoqueFinal}. Atualizando range: ${updateRange}`);
                        ...
                        console.log(`[Planilha - ${codigoService}] SUCESSO: Planilha de estoque atualizada.`);
                        stockUpdateResults.push({ codigo: codigoService, status: 'success', newStock: estoqueFinal });
                        
                        // --- NOTIFICAÇÃO WEBSOCKET ---
                        if (io) {
                            console.log(`[WebSocket] Emitindo atualização de estoque (Saída Fábrica) para o código: ${codigoService}`);
                    ...
                    console.error(`[ERRO GERAL - ${codigoService}] Falha ao dar baixa no estoque:`, error.message);
                    stockUpdateResults.push({ codigo: codigoService, status: 'error', message: error.message });
                }
            }

            console.log('\n--- Processo de baixa de estoque finalizado. ---');
            res.status(200).send({
                status: 'success',
                message: 'Saída registrada e baixas de estoque processadas.',
                data: appendResponse.data,
...
module.exports = createSaidaFabricaRouter;
